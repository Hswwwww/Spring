...
<body>
	...
	<table class="vertical">
		...
		<tr>
			...
			<input type="file" name="attach" style="display:none;"></td>
		</tr>
	</table>
	<div>
		<button>보내기</button>
	...
	<script>
		...
		$('#form2').submit(() => {
			...
			Array.from($('#form2 input[name=attach]')[0].files).forEach(file => {
				...
				//배열인거 같은데 forEach안먹으면 유사배열 > 유사배열을 진짜배열로 만들어줌 > Array.form
				//개별 파일 체크
		...
		.on('drop', e => {
			...
			if (files != null && files != undefined) {
				...
				for (let i=0; i<files.length; i++) {
					...
					filesize = filesize < 1 ? filesize.toFixed(2):filesizze.toFixed(1);
