...
<body>
	...
	<script>
		...
		// 서버측 주소
		//const url = 'ws://echo.websocket.org'; //테스트주소 (echo : 메아리)
		const url = 'ws://192.168.10.34:8080/websocket/server.do';
		// 자바 먼저 갔다가 얘가 온다. 
		// 웹소켓
		let ws;
		...
		$('#btn-connect').click(()=>{
			...
			//1. 소켓 생성 + 2. 서버 접속(연결) > 연결시도
			ws = new WebSocket(url);
			
			//비동기는 언제 상황이 완료될지 모르니까 event 콜백함수로 처리 >
			//얘도 계속 대기만 할 수 없기 때문에 event를 걸어둠 
			//소켓이벤트 > 콜백
